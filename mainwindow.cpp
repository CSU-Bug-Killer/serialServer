    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(singleServer,SIGNAL(readMessageSignal(QString)),
            this,SLOT(server_send_to_serial_port(QString)));//json转16进制到底层
    ...
    //记录设备模式
    connect(ui->logDeviceBtn,SIGNAL(clicked()),
            this,SLOT(setLogDevice()));
    connect(this,SIGNAL(sendMegSignal(QByteArray)),
            this,SLOT(logDevice(QByteArray)));

}

void MainWindow::logDevice(QByteArray data){

    qDebug() << "logDevice:" << data.toHex();

    QByteArray hex = data.toHex();
    QString addr ;
    for(int i = 6;i<10;i++){
        addr.append(hex.at(i));
    }
    qDebug() << addr;
    ...
    QString temp = "";
    temp.append(hex.at(1));
    i = temp.toInt();


    //    QString q = hex.at(1).toString;
    qDebug() << temp;

        if(1==i){
            if(DeviceInfo::carHashset.key(addr).size()==0){
//            DeviceInfo *device = new DeviceInfo(DeviceInfo::DEVICE_TYPE_CAR,addr);
            int i = DeviceInfo::carHashset.size();
            i++;
            QString n = QString::number(i, 10);
            DeviceInfo::carHashset.insert("car"+i,addr);
            QString temp = "";
            temp.append("car");
            temp.append(n),
                    temp.append("    ");
            temp.append(addr);
            ui->logDeviceTdt->append(temp);
            }
        }

        if(0==i){
            if(DeviceInfo::lightHashset.key(addr).size()==0){
//            DeviceInfo *device = new DeviceInfo(DeviceInfo::DEVICE_TYPE_CAR,addr);
            int i = DeviceInfo::lightHashset.size();
            i++;
            QString n = QString::number(i, 10);
            QString key = "light";

            key.append(tr("%1").arg(i));
            qDebug() << key << "   key";

            DeviceInfo::lightHashset.insert(key,addr);
            qDebug() << DeviceInfo::lightHashset.size() << DeviceInfo::lightHashset.value("light1");
            //        ui->logDeviceTdt->append();
            QString temp = "";
...
void MainWindow::setLogDevice(){
    isLogDevice = ui->logDeviceCbx->isChecked();
}
...
void MainWindow::server_send_to_serial_port(QString data)
{
